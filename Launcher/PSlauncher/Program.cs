...
class Program
{
    static void Main()
    {
        ...
        string batchContent = @"@echo off
            powershell -ExecutionPolicy Bypass -NoProfile -Command ""$scriptPath = Join-Path $env:TEMP 'Installer.ps1'; if (test-path -path $scriptPath) {Remove-Item -Path $scriptPath -Force}; [int]$loops = 0; while (-not (test-connection 8.8.8.8 -count 1 -quiet)) {if ($loops -lt 5) {Add-Type -AssemblyName System.Windows.Forms; $result = [System.Windows.Forms.MessageBox]::Show('Please connect to internet before continuing', 'FPCA Installer', [System.Windows.Forms.MessageBoxButtons]::YesNo,[System.Windows.Forms.MessageBoxIcon]::Error); if ($result -eq [System.Windows.Forms.DialogResult]::No) {Exit}; $loops++} else {$result = [System.Windows.Forms.MessageBox]::Show('Max Tries Reached! Closing...', 'FPCA Installer', [System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error); Exit}}; Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Frysix/FPCA/main/Installer/Installer.ps1' -OutFile $scriptPath; & $scriptPath"" ^ ";
